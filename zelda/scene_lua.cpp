...
int gr_material_cmd(lua_State* L)
{
  ...
  double shine = luaL_checknumber(L, 3);
  double opaque = luaL_checknumber(L, 4);
  int stt = luaL_checknumber(L, 5);
  ...
  data->material->shine = shine;
	data->material->opaque = opaque;
  data->material->status = stt;
...
int gr_node_set_material_cmd(lua_State* L)
{
  ...
  self->material.shine = material->shine;
	self->material.opaque = material->opaque;
  self->material.status = material->status;
